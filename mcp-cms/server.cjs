...
// Keep-alive endpoint for Replit (prevents sleep)
...
// Self-ping to keep server alive on Replit
function keepAlive() {
  const REPLIT_URL = process.env.REPLIT_URL || 'https://mcp-cms-server.saifraza91.repl.co';
  
  if (process.env.REPLIT_URL || process.env.NODE_ENV === 'production') {
    setInterval(() => {
      try {
        fetch(`${REPLIT_URL}/keep-alive`)
    ...
    console.log('🏓 Keep-alive system started for Replit');
  }
}
