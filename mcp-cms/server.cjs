...
// Claude content management endpoint
app.post('/api/claude/change-content', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`🧠 Claude request from ${req.user.username}: "${request}"`);
    
    const claudeManager = new ClaudeContentManager();
    const result = await claudeManager.makeContentChange(request, {
...
// Get available Claude commands
app.get('/api/claude/commands', authMiddleware, (req, res) => {
  try {
    const claudeManager = new ClaudeContentManager();
    const commands = claudeManager.getAvailableCommands();
    ...
    res.json({
      ...
      examples: [
        ...
        'Change sugar capacity to 10000 TCD',
        'Add board member Dr. Smith as Independent Director',
